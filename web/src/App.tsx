      ...
      <main className="main">
        ...
        {loading ? <PageLoading /> : error && !jobs.length && !candidates.length ? <ErrorState message={error} retry={() => void loadData()} /> : (
          <>
            ...
            {view === 'job-profile' && selectedJob && <Suspense fallback={<PageLoading />}><JobProfilePage job={selectedJob} onBack={() => go('jobs')} onUpdated={(updated) => setJobs((items) => items.map((item) => item.id === updated.id ? updated : item))} /></Suspense>}
