...
- **动态 JD 岗位画像**：岗位画像 Agent 根据每份 JD 的目标、职责与业务场景动态生成恰好五个维度，名称和权重不套用固定模板，并保留必备项、加分项和核验证据。
- **人工事后校准**：Agent 画像生成后立即生效；招聘人员可修改维度名称、描述、权重、评估要求和关键项，并选择仅影响后续候选人、重评未决策候选人或重评全部候选人。
...
| Agent | 职责 | 关键边界 |
| --- | --- | --- |
| JD 岗位画像 | 根据 JD 动态生成五维画像，并支持人工版本化校准 | 不添加 JD 未表达的门槛，不覆盖人工候选人决策 |
